<template>
    <div>
        <div class="bg-1 h-auto w-full grid grid-cols-6 items-end">
            <div class="col-span-5 px-20 py-10">
                    <h3 class="text-6xl text-white font-bold mb-4 w-1/3 tracking-wider ">MORE ABOUT US</h3>
                    <nuxt-link to="/signed/products">
                    <button class="px-8 my-6 py-3 border-2 text-white border-white font-bold text-xl hover:text-teal-900 hover:border-teal-900 hover:bg-white">
                        Explore Store
                    </button>
                    </nuxt-link>
            </div>
        </div>

        <div class="flex justify-center items-center py-10">
      <div class="grid grid-cols-5 gap-20">
  
        <div class="flex justify-center items-center flex-col">
          <svg class="w-20" fill="rgb(15 118 110)" viewBox="0 0 64 64" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;" version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:serif="http://www.serif.com/" xmlns:xlink="http://www.w3.org/1999/xlink"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g id="ICON"> <path d="M35.596,48.159l-2.631,9.822c-0.114,0.424 0.063,0.873 0.435,1.106c0.372,0.233 0.853,0.196 1.185,-0.091l3.731,-3.231c0,0 1.616,4.663 1.616,4.663c0.143,0.411 0.535,0.683 0.97,0.673c0.434,-0.011 0.813,-0.301 0.935,-0.719l1.673,-5.708l1.652,5.705c0.121,0.418 0.499,0.71 0.935,0.721c0.435,0.012 0.828,-0.26 0.971,-0.672l1.616,-4.663c-0,0 3.731,3.231 3.731,3.231c0.332,0.287 0.813,0.324 1.185,0.091c0.372,-0.233 0.549,-0.682 0.435,-1.106l-2.631,-9.822c1.919,-1.981 3.101,-4.68 3.101,-7.654c-0,-6.074 -4.931,-11.005 -11.005,-11.005c-6.074,-0 -11.005,4.931 -11.005,11.005c0,2.974 1.182,5.673 3.101,7.654Zm1.689,1.426l-1.545,5.765l2.383,-2.063c0.251,-0.217 0.593,-0.296 0.914,-0.21c0.32,0.086 0.577,0.325 0.686,0.639c-0,-0 1.075,3.101 1.075,3.101l1.572,-5.365c-1.874,-0.191 -3.609,-0.854 -5.085,-1.867Zm12.43,0c-1.468,1.008 -3.194,1.669 -5.056,1.864l1.549,5.352l1.069,-3.085c0.109,-0.314 0.366,-0.553 0.686,-0.639c0.321,-0.086 0.663,-0.007 0.914,0.21c-0,0 2.383,2.063 2.383,2.063l-1.545,-5.765Zm-46.715,-15.585l0,15c-0,0.796 0.316,1.559 0.879,2.121c0.562,0.563 1.325,0.879 2.121,0.879c6.483,-0 26.5,-0 26.5,-0c0.552,0 1,-0.448 1,-1c0,-0.552 -0.448,-1 -1,-1c0,-0 -20.017,-0 -26.5,-0c-0.265,0 -0.52,-0.105 -0.707,-0.293c-0.188,-0.187 -0.293,-0.442 -0.293,-0.707l0,-15c0,-0.552 -0.448,-1 -1,-1c-0.552,-0 -1,0.448 -1,1Zm0,-28l0,24c0,0.552 0.448,1 1,1c0.552,-0 1,-0.448 1,-1l0,-18l30,-0c0.552,-0 1,-0.448 1,-1c-0,-0.552 -0.448,-1 -1,-1l-30,-0c0,0 -0,-4 -0,-4c0,-0.552 0.448,-1 1,-1c0,0 51.5,0 51.5,0c0.552,0 1,0.448 1,1c0,0 0,4 0,4c0,-0 -19.5,-0 -19.5,-0c-0.552,-0 -1,0.448 -1,1c-0,0.552 0.448,1 1,1l19.5,-0l-0,37c-0,0.552 -0.448,1 -1,1l-2.5,-0c-0.552,0 -1,0.448 -1,1c0,0.552 0.448,1 1,1l2.5,-0c1.657,0 3,-1.343 3,-3l0,-43c-0,-1.657 -1.343,-3 -3,-3l-51.5,0c-1.657,0 -3,1.343 -3,3Zm40.5,25.5c4.97,0 9.005,4.035 9.005,9.005c-0,4.97 -4.035,9.005 -9.005,9.005c-4.97,-0 -9.005,-4.035 -9.005,-9.005c0,-4.97 4.035,-9.005 9.005,-9.005Zm-14.5,13.5l-0,-2c-0,-1.657 -1.343,-3 -3,-3l-14,-0c-1.657,-0 -3,1.343 -3,3l0,2c0,1.657 1.343,3 3,3l14,0c1.657,0 3,-1.343 3,-3Zm15.374,-12.127c-0.176,-0.317 -0.511,-0.514 -0.874,-0.514c-0.363,0 -0.698,0.197 -0.874,0.514l-1.979,3.559c-0,0 -3.997,0.783 -3.997,0.783c-0.356,0.07 -0.646,0.327 -0.758,0.672c-0.113,0.346 -0.029,0.724 0.218,0.99l2.774,2.982c-0,0 -0.491,4.043 -0.491,4.043c-0.044,0.36 0.111,0.716 0.405,0.93c0.293,0.213 0.68,0.25 1.009,0.097l3.693,-1.716c0,0 3.693,1.716 3.693,1.716c0.329,0.153 0.716,0.116 1.009,-0.097c0.294,-0.214 0.449,-0.57 0.405,-0.93l-0.491,-4.043c0,0 2.774,-2.982 2.774,-2.982c0.247,-0.266 0.331,-0.644 0.218,-0.99c-0.112,-0.345 -0.402,-0.602 -0.758,-0.672l-3.997,-0.783c0,0 -1.979,-3.559 -1.979,-3.559Zm-17.374,10.127l-0,2c-0,0.552 -0.448,1 -1,1l-14,0c-0.552,0 -1,-0.448 -1,-1c-0,-0 0,-2 0,-2c0,-0.552 0.448,-1 1,-1l14,-0c0.552,-0 1,0.448 1,1Zm16.5,-7.583l1.331,2.394c0.143,0.257 0.393,0.438 0.682,0.495l2.688,0.526c-0,0 -1.866,2.006 -1.866,2.006c-0.2,0.215 -0.295,0.509 -0.26,0.801l0.33,2.719c0,-0 -2.484,-1.154 -2.484,-1.154c-0.267,-0.124 -0.575,-0.124 -0.842,-0l-2.484,1.154c-0,-0 0.33,-2.719 0.33,-2.719c0.035,-0.292 -0.06,-0.586 -0.26,-0.801l-1.866,-2.006c0,0 2.688,-0.526 2.688,-0.526c0.289,-0.057 0.539,-0.238 0.682,-0.495l1.331,-2.394Zm-33.5,1.583l18.5,-0c0.552,0 1,-0.448 1,-1c0,-0.552 -0.448,-1 -1,-1l-18.5,-0c-0.552,0 -1,0.448 -1,1c0,0.552 0.448,1 1,1Zm0,-4l21,0c0.552,0 1,-0.448 1,-1c0,-0.552 -0.448,-1 -1,-1l-21,0c-0.552,0 -1,0.448 -1,1c0,0.552 0.448,1 1,1Zm0,-4l23,0c0.552,0 1,-0.448 1,-1c0,-0.552 -0.448,-1 -1,-1l-23,0c-0.552,0 -1,0.448 -1,1c0,0.552 0.448,1 1,1Zm0,-4l43,0c0.552,0 1,-0.448 1,-1c0,-0.552 -0.448,-1 -1,-1l-43,0c-0.552,0 -1,0.448 -1,1c0,0.552 0.448,1 1,1Zm0,-4l43,0c0.552,0 1,-0.448 1,-1c0,-0.552 -0.448,-1 -1,-1l-43,-0c-0.552,0 -1,0.448 -1,1c0,0.552 0.448,1 1,1Zm9,-4l25.5,0c0.552,-0 1,-0.448 1,-1c-0,-0.552 -0.448,-1 -1,-1l-25.5,0c-0.552,-0 -1,0.448 -1,1c-0,0.552 0.448,1 1,1Zm-11,-10.5c0.552,-0 1,0.448 1,1c0,0.552 -0.448,1 -1,1c-0.552,-0 -1,-0.448 -1,-1c0,-0.552 0.448,-1 1,-1Zm4,-0c0.552,-0 1,0.448 1,1c0,0.552 -0.448,1 -1,1c-0.552,-0 -1,-0.448 -1,-1c0,-0.552 0.448,-1 1,-1Zm4,-0c0.552,-0 1,0.448 1,1c0,0.552 -0.448,1 -1,1c-0.552,-0 -1,-0.448 -1,-1c0,-0.552 0.448,-1 1,-1Z"></path> </g> </g></svg>
          <p class=" text-teal-700">10 Years Experience</p>
        </div>
  
        <div class="flex justify-center items-center flex-col">
          <svg class="w-20" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" stroke-width="3" stroke="rgb(15 118 110)" fill="none"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M21.68,42.22H37.17a1.68,1.68,0,0,0,1.68-1.68L44.7,19.12A1.68,1.68,0,0,0,43,17.44H17.61a1.69,1.69,0,0,0-1.69,1.68l-5,21.42a1.68,1.68,0,0,0,1.68,1.68h2.18"></path><path d="M41.66,42.22H38.19l5-17.29h8.22a.85.85,0,0,1,.65.3l3.58,6.3a.81.81,0,0,1,.2.53L52.51,42.22h-3.6"></path><ellipse cx="18.31" cy="43.31" rx="3.71" ry="3.76"></ellipse><ellipse cx="45.35" cy="43.31" rx="3.71" ry="3.76"></ellipse><line x1="23.25" y1="22.36" x2="6.87" y2="22.36" stroke-linecap="round"></line><line x1="20.02" y1="27.6" x2="8.45" y2="27.6" stroke-linecap="round"></line><line x1="21.19" y1="33.5" x2="3.21" y2="33.5" stroke-linecap="round"></line></g></svg>
          <p class=" text-teal-700">Flexible Delivery</p>
        </div>
        
        <div class="flex justify-center items-center flex-col">
          <svg class="w-20" viewBox="0 0 64 64" data-name="Layer 1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" fill="rgb(15 118 110)"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><defs><style>.cls-1{fill:rgb(15 118 110);}</style></defs><title></title><path class="cls-1" d="M14.73,42.9a5,5,0,0,0,1.35.94,30.35,30.35,0,0,0,2.12,9.71,10.2,10.2,0,0,1,.8,3.91V61a3,3,0,0,0,3,3H34a3,3,0,0,0,3-3,7.53,7.53,0,0,1,1.66-4.71l6.53-8.14c.11-.12.21-.25.31-.37s.2-.21.29-.33.19-.27.28-.41l.25-.36c.09-.15.15-.3.23-.44s.14-.26.2-.39.13-.32.19-.48l.15-.39c0-.17.09-.34.13-.52s.07-.25.1-.38,0-.37.07-.55,0-.25.06-.38,0-.42,0-.63,0-.2,0-.31a7.47,7.47,0,0,0-.09-.94h0l-.08-.48-1-6.48a8,8,0,0,0-.6-2l0,0a.67.67,0,0,0-.06-.12,8,8,0,0,0-4.45-4.08L38.05,27l3.47-3.4L45,20a1,1,0,0,1,.9-.26,10,10,0,0,0,11.64-6.89,9.91,9.91,0,0,0,.37-4,2,2,0,0,0-3.42-1.17l-2.78,2.84a1,1,0,0,1-1.42,0L47.49,7.73A1,1,0,0,1,47.2,7a1,1,0,0,1,.3-.72l2.79-2.84a2,2,0,0,0,.47-2.08A2,2,0,0,0,49.11.06a10.16,10.16,0,0,0-4,.38,10,10,0,0,0-6.84,11.63A1,1,0,0,1,38,13L32.18,18.7a4,4,0,0,0-7.09,1.55A4,4,0,0,0,20,24.1a3.85,3.85,0,0,0,.25,1.39l-.24,0a4,4,0,0,0-2.92,5.79,4,4,0,0,0-2.66,4.89,5,5,0,0,0,.3,6.7Zm.83-4.83.18.13a2.42,2.42,0,0,0,.31.25,4.14,4.14,0,0,0,.62.35l4,1.72a3.58,3.58,0,0,0,.55.19l-.81.8a3.12,3.12,0,0,1-.41.33l-.14.09-.33.17-.18.08-.38.11-.29,0-.16,0a3.22,3.22,0,0,1-1.14-.14h0a2.91,2.91,0,0,1-1.17-.71,3,3,0,0,1-.59-3.42ZM42.44,31.2a6,6,0,0,1,1.82,3.43l1.13,7a5.69,5.69,0,0,1,.07.71,6.07,6.07,0,0,1-1.73,4.48l-.07.07L37.1,55A9.5,9.5,0,0,0,35,61a1,1,0,0,1-1,1H22a1,1,0,0,1-1-1V57.46a12.1,12.1,0,0,0-1-4.68,28.18,28.18,0,0,1-1.94-8.43h.17a4.34,4.34,0,0,0,.62-.06,2.11,2.11,0,0,0,.25,0,4.26,4.26,0,0,0,.7-.19l.25-.07a5.56,5.56,0,0,0,.73-.37l.16-.08a5.2,5.2,0,0,0,.79-.63l5.61-5.51a3.75,3.75,0,0,0,1.7-1l.06-.05a4.91,4.91,0,0,0,.41-.48,3.81,3.81,0,0,0,.66-1.5,4.13,4.13,0,0,0,.93-.09,6.84,6.84,0,0,0,2.56,1.58l2.38.91a1.51,1.51,0,0,1,1,1.24,1.55,1.55,0,0,1-1,1.62c-5.36,2-8.39,5.44-9,10.19A1,1,0,0,0,27.87,51H28a1,1,0,0,0,1-.87c.54-4,3.07-6.82,7.74-8.57A3.57,3.57,0,0,0,39,37.82,3.5,3.5,0,0,0,36.79,35L34.38,34a5,5,0,0,1-2.22-1.55,3.3,3.3,0,0,1-.34-.49c-.06-.09-.12-.18-.17-.27a4.41,4.41,0,0,1-.28-.58c-.05-.13-.1-.26-.14-.4s-.07-.25-.1-.37-.05-.3-.07-.45a2.58,2.58,0,0,1,0-.39.48.48,0,0,1,0-.12,3.37,3.37,0,0,1,0-.55,1.36,1.36,0,0,1,0-.2.78.78,0,0,1,0-.16,4.86,4.86,0,0,1,.19-.82,1,1,0,0,1,1.25-.63L40.42,30A6.15,6.15,0,0,1,42.44,31.2ZM39.37,14.38a3,3,0,0,0,.81-2.73A7.78,7.78,0,0,1,40.26,8a7.93,7.93,0,0,1,5.38-5.64,7.76,7.76,0,0,1,3.23-.3L46.08,4.9a3,3,0,0,0,0,4.25L48.91,12a3.08,3.08,0,0,0,4.25,0L56,9.11h0a8.12,8.12,0,0,1-.29,3.2A7.9,7.9,0,0,1,50,17.74a7.77,7.77,0,0,1-3.68.08,3,3,0,0,0-2.74.81l-7.1,7,0-.06a3.65,3.65,0,0,0-.35-.75h0L33.3,20.41Zm-11.44,5a2,2,0,0,1,2.76.62l.49.77h0l3,4.74-1-.37a2.94,2.94,0,0,0-2.29.15,2.61,2.61,0,0,0-.28.17l-3.55-3.95A2,2,0,0,1,27.93,19.4Zm-5.34,3.29a2,2,0,0,1,2.85,0l3.92,4.37c-.05.16-.09.32-.14.49a7,7,0,0,0,.42,4.63l-5.45-5.09-1.6-1.6a2,2,0,0,1,0-2.82ZM19,28.3a2,2,0,0,1,2.79-.45l1,.76,5.45,5.07v0a2,2,0,0,1-.37,1l0,0a1.79,1.79,0,0,1-.22.25l0,0a2,2,0,0,1-1,.48,2,2,0,0,1-1.08-.08l-5.41-3.8h0l-.75-.55A2,2,0,0,1,19,28.3Zm-2.59,6a2,2,0,0,1,1.84-1.2,2,2,0,0,1,.8.16l.08,0,5.06,3.55a2,2,0,0,1-.15.77A1.94,1.94,0,0,1,23,38.71a2,2,0,0,1-1.53,0l-4-1.73,0,0a1.77,1.77,0,0,1-.33-.19l0,0a2.17,2.17,0,0,1-.6-.75h0l-.06-.11A2,2,0,0,1,16.43,34.33Z"></path><path class="cls-1" d="M25.55,11.78a.51.51,0,0,0,.23.67l2,1a.54.54,0,0,0,.22.05.5.5,0,0,0,.22-.95l-2-1A.51.51,0,0,0,25.55,11.78Z"></path><path class="cls-1" d="M29,10.5a.47.47,0,0,0,.35-.15.48.48,0,0,0,0-.7l-2-2a.49.49,0,0,0-.7.7l2,2A.47.47,0,0,0,29,10.5Z"></path><path class="cls-1" d="M32.5,10V9a.5.5,0,0,0-1,0v1a.5.5,0,0,0,1,0Z"></path></g></svg>
          <p class=" text-teal-700">Free Installation</p>
        </div>
  
        <div class="flex justify-center items-center flex-col">
          <svg class="w-20" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="rgb(15 118 110)"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>support</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="support" fill="rgb(15 118 110)" transform="translate(42.666667, 42.666667)"> <path d="M379.734355,174.506667 C373.121022,106.666667 333.014355,-2.13162821e-14 209.067688,-2.13162821e-14 C85.1210217,-2.13162821e-14 45.014355,106.666667 38.4010217,174.506667 C15.2012632,183.311569 -0.101643453,205.585799 0.000508304259,230.4 L0.000508304259,260.266667 C0.000508304259,293.256475 26.7445463,320 59.734355,320 C92.7241638,320 119.467688,293.256475 119.467688,260.266667 L119.467688,230.4 C119.360431,206.121456 104.619564,184.304973 82.134355,175.146667 C86.4010217,135.893333 107.307688,42.6666667 209.067688,42.6666667 C310.827688,42.6666667 331.521022,135.893333 335.787688,175.146667 C313.347976,184.324806 298.68156,206.155851 298.667688,230.4 L298.667688,260.266667 C298.760356,283.199651 311.928618,304.070103 332.587688,314.026667 C323.627688,330.88 300.801022,353.706667 244.694355,360.533333 C233.478863,343.50282 211.780225,336.789048 192.906491,344.509658 C174.032757,352.230268 163.260418,372.226826 167.196286,392.235189 C171.132153,412.243552 188.675885,426.666667 209.067688,426.666667 C225.181549,426.577424 239.870491,417.417465 247.041022,402.986667 C338.561022,392.533333 367.787688,345.386667 376.961022,317.653333 C401.778455,309.61433 418.468885,286.351502 418.134355,260.266667 L418.134355,230.4 C418.23702,205.585799 402.934114,183.311569 379.734355,174.506667 Z M76.8010217,260.266667 C76.8010217,269.692326 69.1600148,277.333333 59.734355,277.333333 C50.3086953,277.333333 42.6676884,269.692326 42.6676884,260.266667 L42.6676884,230.4 C42.6676884,224.302667 45.9205765,218.668499 51.2010216,215.619833 C56.4814667,212.571166 62.9872434,212.571166 68.2676885,215.619833 C73.5481336,218.668499 76.8010217,224.302667 76.8010217,230.4 L76.8010217,260.266667 Z M341.334355,230.4 C341.334355,220.97434 348.975362,213.333333 358.401022,213.333333 C367.826681,213.333333 375.467688,220.97434 375.467688,230.4 L375.467688,260.266667 C375.467688,269.692326 367.826681,277.333333 358.401022,277.333333 C348.975362,277.333333 341.334355,269.692326 341.334355,260.266667 L341.334355,230.4 Z"> </path> </g> </g> </g></svg>
          <p class=" text-teal-700">After Sales Support</p>
        </div>
  
        <div class="flex justify-center items-center flex-col">
          <svg class="w-20" fill="rgb(15 118 110)" viewBox="0 0 52 52" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M26,2c3,0,5.43,3.29,8.09,4.42s6.82.51,8.84,2.65,1.51,6.07,2.65,8.84S50,23,50,26s-3.29,5.43-4.42,8.09-.51,6.82-2.65,8.84-6.07,1.53-8.84,2.65S29,50,26,50s-5.43-3.29-8.09-4.42-6.82-.51-8.84-2.65-1.53-6.07-2.65-8.84S2,29,2,26s3.29-5.43,4.42-8.09.51-6.82,2.65-8.84,6.07-1.53,8.84-2.65S23,2,26,2Zm0,7.58A16.42,16.42,0,1,0,42.42,26h0A16.47,16.47,0,0,0,26,9.58Zm7.62,9.15,1.61,1.52a1.25,1.25,0,0,1,0,1.51L25.08,33.07a2.07,2.07,0,0,1-1.61.7,2.23,2.23,0,0,1-1.61-.7L16.37,27.6a1,1,0,0,1-.1-1.42l.1-.11L18,24.56a1.1,1.1,0,0,1,1.54-.07l.07.07,3.89,4,8.59-9.8A1.1,1.1,0,0,1,33.62,18.73Z"></path></g></svg>
          <p class=" text-teal-700">5 Years Warranty</p>
        </div>
      </div>
    </div>

    
    <div class="bg-2 grid grid-cols-2 ">
        <div class=""></div>
        <div class="bg-gray-300 grid grid-rows-4 p-10 gap-8">
            <div class="flex">
                <p class="text-teal-800 font-bold text-3xl">Permanent Exhibitions</p>
            </div>
            <div class="flex gap-4">
                <div class="flex justify-center items-center">
                    <img class="w-20" src="https://websitedemos.net/furniture-store-04/wp-content/uploads/sites/155/elementor/thumbs/chair-p3wh9r1b8i4eszqxlj7km84pe28n4f4y8lc4pov5g8.png" alt="">
                </div>
                <div class="">
                    <h3 class="font-bold text-xl text-teal-600">Unique & Minimal Design</h3>
                    <p class="text-teal-600">Quae semper modi tenetur sem suscipit corporis. Cupidatat facilisis pariatur venena.</p>
                </div>
            </div>

            <div class="flex gap-4">
                <div class="flex justify-center items-center">
                    <img class="w-20" src="https://websitedemos.net/furniture-store-04/wp-content/uploads/sites/155/2021/03/sofa.png" alt="">
                </div>
                <div class="">
                    <h3 class="font-bold text-xl text-teal-600">Unique & Minimal Design</h3>
                    <p class="text-teal-600">Quae semper modi tenetur sem suscipit corporis. Cupidatat facilisis pariatur venena.</p>
                </div>
            </div>

            <div class="flex gap-4">
                <div class="flex justify-center items-center">
                    <img class="w-20" src="https://websitedemos.net/furniture-store-04/wp-content/uploads/sites/155/elementor/thumbs/sofa-chair-p3whoooztqkf9y1ydzm68ggd6gmlfafeyihv7ypyl4.png" alt="">
                </div>
                <div class="">
                    <h3 class="font-bold text-xl text-teal-600">Crafted Carefully</h3>
                    <p class="text-teal-600">Quae semper modi tenetur sem suscipit corporis. Cupidatat facilisis pariatur venena.</p>
                </div>
            </div>
        </div>
    </div>


    <div class="grid grid-cols-2 pt-12">
        <div class="flex px-20 flex-col gap-10 justify-center bg-gray-300">
            <h3 class="text-teal-800 font-bold text-3xl">Repellat Montes!</h3>
            <p class="text-teal-600">Quisque vitae dapibus leo. Morbi dictum, lectus ut scelerisque mattis, eros lacus mollis metus.</p>
        </div>
        <div class="w-full h-full">
            <img src="https://websitedemos.net/furniture-store-04/wp-content/uploads/sites/155/2020/02/home-sofa-set.jpg" alt="">
        </div>
    </div>

    <div class="flex justify-center items-center gap-4 flex-col py-10 mt-10 bg-gray-200">
      <div class="flex flex-col justify-center items-center gap-5 pb-5">
        <h3 class="w-full text-2xl font-bold text-stone-600 text-center">Testimonials</h3>
        <hr class="w-full">
      </div>
      <div class="grid grid-cols-3 gap-10 px-24">
        <div class="flex justify-start items-center flex-col gap-4">
          <h3 class="h-24">"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo"</h3>
          <div class="flex justify-center items-center gap-5">
            <img class="w-14" src="https://websitedemos.net/furniture-store-04/wp-content/uploads/sites/155/2018/01/pic19-free-img.png" alt="">
            <p>Patricia Warren</p>
          </div>
        </div>
        <div class="flex justify-start items-center flex-col gap-4">
          <h3 class="h-24">"Convallis lacinia optio! Hac morbi laboris deleniti, hymenaeos, nihil, magnis corrupti nisl dis dolores aptent eveniet nostrud eu"</h3>
          <div class="flex justify-center items-center gap-5">
            <img class="w-14" src="https://websitedemos.net/furniture-store-04/wp-content/uploads/sites/155/2018/01/clients01-free-img.png" alt="">
            <p>Lauren Lane</p>
          </div>
        </div>
        <div class="flex justify-start items-center flex-col gap-4">
          <h3 class="h-24">"Semper laboris possimus, molestiae, anim ornare? Molestie! Dictumst scelerisque error, autem leo. Autem integer diamlorem"</h3>
          <div class="flex justify-center items-center gap-5">
            <img class="w-14" src="https://websitedemos.net/furniture-store-04/wp-content/uploads/sites/155/2018/01/pic20-free-img.png" alt="">
            <p>Paul Smitten</p>
          </div>
        </div>
      </div>
    </div>
  
        
    </div>
</template>

<script>
export default {
    layout: 'signedLayout',
}
</script>


<style scoped>
    .bg-1{
        background: url('https://www.visithastingsmn.org/wp-content/uploads/2020/02/hanging-lamp-banner-1024x349.jpg');
        background-position: center;
        background-repeat: no-repeat;
        background-size: cover;
    }
    .bg-2{
        background: url('https://cdn.pixabay.com/photo/2020/08/25/18/28/workplace-5517744_640.jpg');
        background-repeat: no-repeat;
        background-size: contain;
    }

</style>